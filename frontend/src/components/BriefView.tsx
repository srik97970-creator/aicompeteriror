...
import { ComparisonBrief } from '../types';
...
export default function BriefView({
  ...
  return (
    <div className="space-y-8 animate-slide-up" ref={printRef}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 no-print bg-slate-900/40 border border-slate-800 p-4 rounded-2xl">
        <div className="flex items-center gap-2">
          ...
          {speakingText && (
            <button
              ...
              className="ml-3 flex items-center gap-1 px-2.5 py-1 bg-red-500/10 border border-red-500/20 text-red-400 rounded-full text-[10px] font-bold animate-pulse"
            >
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold rounded-xl transition-all cursor-pointer"
          >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 space-y-8 print-card shadow-2xl">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => handleSpeak(brief.executiveSummary)}
              className={`p-1.5 rounded-lg flex items-center gap-1 text-[10px] font-bold transition-all border no-print cursor-pointer ${
                speakingText === brief.executiveSummary
          ...
          <div className="overflow-x-auto border border-slate-800/80 rounded-2xl">
            <table className="min-w-full divide-y divide-slate-800 text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-800 text-slate-300">
                {brief.comparisonTable.map((row, index) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => handleSpeak(getAdvantagesSpeechText())}
                className={`p-1.5 rounded-lg flex items-center gap-1 text-[10px] font-bold transition-all border no-print cursor-pointer ${
                  speakingText === getAdvantagesSpeechText()
                    ? 'bg-red-500/20 text-red-500 border-red-500/30'
                    : 'bg-slate-950/60 border-slate-800 text-slate-400 hover:text-indigo-450'
                }`}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => handleSpeak(getTalkingPointsSpeechText())}
                className={`p-1.5 rounded-lg flex items-center gap-1 text-[10px] font-bold transition-all border no-print cursor-pointer ${
                  speakingText === getTalkingPointsSpeechText()
                    ? 'bg-red-500/20 text-red-500 border-red-500/30'
                    : 'bg-slate-950/60 border-slate-800 text-slate-400 hover:text-indigo-450'
                }`}
              >
            ...
            <div className="space-y-3">
              {brief.talkingPoints.map((tp, i) => (
                <div key={i} className="bg-slate-950/30 border-l-2 border-indigo-500 p-3 rounded-r-xl">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => handleSpeak(getObjectionsSpeechText())}
                className={`p-1.5 rounded-lg flex items-center gap-1 text-[10px] font-bold transition-all border no-print cursor-pointer ${
                  speakingText === getObjectionsSpeechText()
                    ? 'bg-red-500/20 text-red-500 border-red-500/30'
                    : 'bg-slate-950/60 border-slate-800 text-slate-400 hover:text-indigo-450'
                }`}
              >
            ...
            <div className="space-y-4">
              {brief.objectionHandling.map((item, i) => (
                <div key={i} className="bg-slate-950/40 border border-slate-850 p-3.5 rounded-2xl space-y-1.5">
        ...
        <div className="bg-indigo-950/10 border border-indigo-500/25 p-5 rounded-2xl space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => handleSpeak(brief.recommendation)}
              className={`p-1.5 rounded-lg flex items-center gap-1 text-[10px] font-bold transition-all border no-print cursor-pointer ${
                speakingText === brief.recommendation
      ...
      <div className="no-print bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-4 max-w-2xl mx-auto shadow-xl">
        ...
        {feedbackSubmitted ? (
          <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-xl p-4 text-xs font-semibold text-center">
            ✔ {t('feedbackSuccess')}
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-1">
              {[1, 2, 3, 4, 5].map((stars) => (
                <button
                  ...
                  className="p-1 rounded-lg text-slate-650 hover:bg-slate-850 transition-colors cursor-pointer"
                >
                  <Star 
                    className={`w-6 h-6 ${
            ...
            <div>
              <textarea
                ...
                className="w-full bg-slate-950/60 border border-slate-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl p-3 text-xs text-slate-105 placeholder-slate-500 focus:outline-none min-h-[70px]"
                placeholder="Optional comments on adjustments needed (e.g. key specifications to highlight)..."
              />
            </div>
            ...
            <button
              ...
              className="px-4 py-2.5 bg-indigo-650 hover:bg-indigo-755 disabled:opacity-50 text-white text-xs font-bold rounded-xl transition-colors shrink-0 cursor-pointer"
            >
