...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col md:flex-row">
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-[53px] bg-slate-950/95 backdrop-blur-md z-40 flex flex-col p-6 border-t border-slate-800">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 border-r border-slate-800 shrink-0">
          ...
          {filteredNavItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-205 ${active
        ...
        <div className="px-4 mb-2">
          <div className="flex items-center gap-2 px-3 py-2 bg-slate-950/40 border border-slate-800/60 rounded-xl">
            ...
            <select
              value={currentLanguage}
              onChange={(e) => setLanguage(e.target.value as any)}
              className="block w-full bg-transparent text-xs text-slate-300 focus:outline-none cursor-pointer"
            >
              {SUPPORTED_LANGUAGES.map(lang => (
        ...
        <div className="px-4 mb-4">
          <button
            onClick={toggleTheme}
            className="w-full flex items-center justify-between px-3 py-2 bg-slate-950/40 border border-slate-800/60 rounded-xl text-xs text-slate-300 hover:bg-slate-800/50 hover:text-white transition-all duration-200 cursor-pointer"
          >
            <div className="flex items-center gap-2">
              {theme === 'dark' ? (
                <>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/50">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-3 py-2 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all w-full text-xs font-semibold"
          >
