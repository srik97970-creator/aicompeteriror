...
import { api } from '../api';
import { User, Product, Template, AIPrompt } from '../types';
...
type TabType = 'users' | 'products' | 'templates' | 'prompts';
...
export default function AdminPanel() {
  ...
  const [promptsList, setPromptsList] = useState<AIPrompt[]>([]);

  // Create Form States - Users
  const [newUserName, setNewUserName] = useState('');
  const [newUserEmail, setNewUserEmail] = useState('');
  const [newUserPassword, setNewUserPassword] = useState('');
  const [newUserRole, setNewUserRole] = useState<'admin' | 'sales_staff'>('sales_staff');
  ...
  const loadTabData = async (tab: TabType) => {
    ...
    try {
      if (tab === 'users') {
        ...
        const data = await api.get<AIPrompt[]>('/api/prompts');
        setPromptsList(data);
      }
    } catch (err: any) {
  ...
  const handleTogglePromptActive = async (id: number, currentActive: boolean) => {
    if (currentActive) return; // Cannot deactivate active prompt directly (must activate another)
    try {
                ...
                <form onSubmit={handleCreateUser} className="bg-slate-950/40 border border-slate-850 p-5 rounded-2xl space-y-4 max-w-2xl">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <input
                      ...
                      className="bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none"
                    ...
                    <input
                      ...
                      className="bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none"
                      placeholder="Email (name@nethigupta.com)"
                    />
                    <input
                      type="password"
                      required
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
                    <div className="flex items-center gap-4 text-xs text-slate-400">
                      ...
                      <label className="flex items-center gap-1.5 select-none cursor-pointer">
                        <input
                          type="radio"
                          name="role"
                          checked={newUserRole === 'sales_staff'}
                          onChange={() => setNewUserRole('sales_staff')}
                    ...
                    <button
                      type="submit"
                      className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl transition-colors shrink-0"
                    >
                      Add Account
                    </button>
                  </div>
                        ...
                        {usersList.map((user) => (
                          <tr key={user.id} className="hover:bg-slate-950/20 transition-colors">
                            ...
                            <td className="px-4 py-3.5">
                              <span className={`px-2 py-0.5 rounded-full font-extrabold text-[9px] uppercase tracking-wide border ${
                                user.role === 'admin'
                            ...
                            <td className="px-4 py-3.5 text-center">
                              <button
                                ...
                                className="p-1 text-red-500 hover:bg-red-500/10 rounded-lg disabled:opacity-30 transition-colors"
                                title={loggedInUser?.id === user.id ? 'You cannot delete yourself' : 'Delete user'}
                              >
                ...
                <form onSubmit={handleCreateProduct} className="bg-slate-950/40 border border-slate-855 p-5 rounded-2xl space-y-4 max-w-2xl">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g. Sole Fitness F63"
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl p-3 text-xs text-slate-200 focus:outline-none min-h-[80px]"
                      placeholder="List motor specs, frame material, bluetooth settings, warranty details to autocomplete in comparison fields..."
                    />
                  </div>
                  ...
                  <div className="flex justify-end pt-2">
                    <button
                      type="submit"
                      className="px-4 py-2.5 bg-indigo-650 hover:bg-indigo-750 text-white font-bold text-xs rounded-xl transition-colors shrink-0"
                    >
                      Save Product
                    </button>
                  </div>
                ...
                <form onSubmit={handleCreateTemplate} className="bg-slate-950/40 border border-slate-855 p-5 rounded-2xl space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-slate-905 bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none"
                        placeholder="e.g. Gym Treadmill comparison"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        required
                        value={newTempCompProd}
                        onChange={(e) => setNewTempCompProd(e.target.value)}
                        className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none"
                        placeholder="e.g. Carbon T7"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        value={newTempCompBrand}
                        onChange={(e) => setNewTempCompBrand(e.target.value)}
                        className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none"
                        placeholder="e.g. ProForm"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        type="text"
                        required
                        value={newTempOurProd}
                        onChange={(e) => setNewTempOurProd(e.target.value)}
                        className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none"
                        placeholder="e.g. Sole Fitness F63"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        value={newTempCustReq}
                        onChange={(e) => setNewTempCustReq(e.target.value)}
                        className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none"
                        placeholder="e.g. concerned about warranty, budget"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <textarea
                        value={newTempCompFeat}
                        onChange={(e) => setNewTempCompFeat(e.target.value)}
                        className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl p-3 text-xs text-slate-200 focus:outline-none min-h-[60px]"
                        placeholder="e.g. 10% incline, requires subscriptions"
                      />
                    </div>
                    <div>
                      <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider mb-1">Our Product Features Details</label>
                      <textarea
                        value={newTempOurFeat}
                        onChange={(e) => setNewTempOurFeat(e.target.value)}
                        className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl p-3 text-xs text-slate-200 focus:outline-none min-h-[60px]"
                        placeholder="e.g. 15% incline, lifetime motor warranty"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none"
                      placeholder="e.g. customer values lifetime parts security"
                    />
                  </div>
                ...
                <form onSubmit={handleCreatePrompt} className="bg-slate-950/40 border border-slate-855 p-5 rounded-2xl space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none"
                      placeholder="e.g. Bold Floor Persuasive Prompt"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl p-3 text-xs text-slate-250 focus:outline-none min-h-[120px] leading-relaxed"
                      placeholder="Specify tone instructions, tables guidelines, talking points parameters, and custom objection constraints..."
                    />
                  </div>
                  ...
                  <div className="flex justify-end pt-2">
                    <button
                      type="submit"
                      className="px-4 py-2.5 bg-indigo-650 hover:bg-indigo-755 text-white font-bold text-xs rounded-xl transition-colors shrink-0"
                    >
                      Save AI Prompt
                    </button>
                  </div>
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex items-center gap-3">
                            <button
                              onClick={() => handleTogglePromptActive(prompt.id, prompt.isActive)}
                              className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-[10px] font-bold border transition-colors cursor-pointer ${
                                prompt.isActive
                            ...
                            <button
                              ...
                              className="p-1.5 text-red-500 hover:bg-red-500/10 border border-transparent rounded-lg disabled:opacity-30 transition-colors"
                              title={prompt.isActive ? 'Active prompt cannot be deleted' : 'Delete prompt'}
                            >
                              <Trash2 className="w-4 h-4" />
                            </button>
                          </div>
