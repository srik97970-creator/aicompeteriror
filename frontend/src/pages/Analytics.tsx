    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map(n => (
            <div key={n} className="h-28 bg-slate-900/40 border border-slate-800 rounded-2xl animate-pulse" />
          ))}
        </div>
    ...
    <div className="space-y-8">
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 space-y-4 shadow-md">
          ...
          <div className="h-64 text-[10px]">
            {charts.daily.length === 0 ? (
              <div className="h-full flex items-center justify-center text-slate-600">No activity data recorded in the last 7 days.</div>
            ) : (
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 space-y-4 shadow-md">
          ...
          <div className="h-64 text-[10px]">
            {charts.monthly.length === 0 ? (
              <div className="h-full flex items-center justify-center text-slate-600">No monthly logs recorded.</div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 space-y-4 shadow-md lg:col-span-2">
          ...
          <div className="h-64 text-[10px]">
            {!charts.topCompetitor || charts.topCompetitor.length === 0 ? (
              <div className="h-full flex items-center justify-center text-slate-650">No comparisons logged yet.</div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 space-y-4 shadow-md">
          ...
          <div className="h-64 text-[10px]">
            {!quality || quality.distribution.length === 0 ? (
              <div className="h-full flex items-center justify-center text-slate-650">No feedback submitted.</div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 space-y-4 shadow-md">
          ...
          <div className="h-60 text-[9px] flex items-center justify-center">
            {!trends || trends.competitorBrands.length === 0 ? (
              <div className="text-slate-650">No brand data available.</div>
            ) : (
              <div className="w-full h-full flex flex-col items-center justify-center">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        ...
                        dataKey="count"
                      >
                        {trends.competitorBrands.map((_: any, index: number) => (
                ...
                <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 mt-1 text-[10px] text-slate-400">
                  {trends.competitorBrands.map((brand: any, idx: number) => (
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 space-y-4 shadow-md lg:col-span-2">
          ...
          <div className="space-y-3.5 max-h-56 overflow-y-auto pr-1">
            {!quality || quality.comments.length === 0 ? (
              <div className="py-8 text-center text-slate-600 text-xs">No feedback comments recorded.</div>
            ) : (
              quality.comments.map((comment: any) => (
                <div key={comment.id} className="bg-slate-950/40 border border-slate-850 p-3 rounded-xl space-y-2 text-xs">
