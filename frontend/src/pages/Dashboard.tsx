...
import { AnalyticsResponse, Template, Comparison } from '../types';
import {
  ResponsiveContainer,
  AreaChart,
  Area,
...
export default function Dashboard() {
  ...
  const [recentBriefs, setRecentBriefs] = useState<Comparison[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const analyticsData = await api.get<AnalyticsResponse>('/api/analytics');
        setAnalytics(analyticsData);

        const temps = await api.get<Template[]>('/api/templates');
        setTemplates(temps);

        const briefsRes = await api.get<{ comparisons: Comparison[] }>('/api/comparison/history?limit=5');
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden">
        ...
        <Link
          to="/generate"
          className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-6 py-3.5 rounded-2xl shadow-lg shadow-indigo-600/20 hover:shadow-indigo-600/30 transition-all flex items-center gap-2 group shrink-0"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-slate-900/30 border border-slate-850 rounded-3xl p-6 space-y-4">
          ...
          <div className="h-60 text-[10px]">
            {loading ? (
              <div className="h-full bg-slate-900/20 rounded-2xl animate-pulse" />
            ) : !analytics || analytics.charts.daily.length === 0 ? (
              <div className="h-full flex items-center justify-center text-slate-600">No chart details recorded.</div>
            ) : (
        ...
        <div className="bg-slate-900/30 border border-slate-850 rounded-3xl p-6 space-y-4">
          ...
          <div className="divide-y divide-slate-800 space-y-3.5">
            {loading ? (
              [1, 2, 3].map(n => (
                <div key={n} className="py-2.5 h-12 bg-slate-900/20 rounded-xl animate-pulse" />
              ))
              ...
              recentBriefs.map((brief) => (
                <div key={brief.id} className="py-3 first:pt-0 last:pb-0 flex items-start justify-between group">
                  <div className="min-w-0 pr-2">
                    <Link
                      to={`/generate?open=${brief.id}`}
                      className="font-bold text-slate-350 hover:text-indigo-400 transition-colors text-sm truncate block"
                    >
                    ...
                    <div className="flex items-center gap-2 mt-1 text-[10px] text-slate-550">
                      ...
                      {brief.rating !== null && brief.rating !== undefined && (
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[1, 2, 3].map(n => (
              <div key={n} className="bg-slate-900/20 border border-slate-850 h-36 rounded-2xl animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {templates.map((temp) => {
              ...
              return (
                <div
                  key={temp.id}
                  onClick={() => handleSelectPreset(temp.id)}
                  className="bg-slate-900/40 hover:bg-slate-900/90 border border-slate-850 hover:border-indigo-500/40 rounded-2xl p-5 cursor-pointer transition-all duration-300 hover:shadow-lg hover:shadow-indigo-500/5 flex flex-col justify-between group relative overflow-hidden"
                >
                  <div>
