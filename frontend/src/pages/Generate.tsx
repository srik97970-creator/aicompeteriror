...
export default function Generate() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const temps = await api.get<Template[]>('/api/templates');
        setTemplates(temps);

        const prods = await api.get<Product[]>('/api/products');
    ...
    const loadReport = async (id: string) => {
      ...
      setValidationError('');
      try {
        const report = await api.get<any>(`/api/comparison/${id}`);
  ...
  useEffect(() => {
    if (currentLanguage === 'hi') setBriefLanguage('Hindi');
    else if (currentLanguage === 'te') setBriefLanguage('Telugu');
    else setBriefLanguage('English');
  }, [currentLanguage]);
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const data: any = await api.post('/api/comparison/generate', payload);
  ...
  const handleSaveAsTemplate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newTemp = await api.post<Template>('/api/templates', payload);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="no-print flex items-center justify-between">
        ...
        {generatedBrief && (
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 border border-slate-800 hover:bg-slate-900 rounded-xl text-xs font-semibold text-slate-300 transition-colors cursor-pointer"
          >
      ...
      {!loading && generatedBrief && currentReportId && (
        <BriefView
          brief={generatedBrief}
          reportId={currentReportId}
          onRegenerate={() => handleGenerate()}
          staffName={user?.name || 'Sales Professional'}
          competitorName={competitorBrand ? `${competitorBrand} ${competitorProduct}` : competitorProduct}
          ourName={ourProduct}
          initialRating={reportRating}
        />
      )}

      {/* Input Entry Form */}
      {!loading && !generatedBrief && (
        <form onSubmit={handleGenerate} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 space-y-8 shadow-xl">
          ...
          <div className="bg-slate-950/60 border border-slate-850 p-4 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="w-full sm:max-w-xs">
              <select
                ...
                defaultValue=""
                className="block w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-300 focus:outline-none"
              >
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      type="button"
                      onClick={() => handleDictation('competitorBrand', competitorBrand, setCompetitorBrand)}
                      className={`p-1 rounded-lg transition-colors ${
                        recordingField === 'competitorBrand' ? 'bg-red-500/20 text-red-500 animate-pulse' : 'text-slate-500 hover:text-indigo-400'
                      }`}
                      title="Speak brand name"
                    >
                  ...
                  <input
                    ...
                    className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none"
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      type="button"
                      onClick={() => handleDictation('competitorProduct', competitorProduct, setCompetitorProduct)}
                      className={`p-1 rounded-lg transition-colors ${
                        recordingField === 'competitorProduct' ? 'bg-red-500/20 text-red-500 animate-pulse' : 'text-slate-500 hover:text-indigo-400'
                      }`}
                  ...
                  <input
                    ...
                    className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none font-semibold"
                    placeholder="e.g. Peloton Bike"
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    type="button"
                    onClick={() => handleDictation('competitorFeatures', competitorFeatures, setCompetitorFeatures)}
                    className={`px-2 py-1 rounded-lg flex items-center gap-1 text-[10px] font-bold border transition-colors ${
                      recordingField === 'competitorFeatures'
                ...
                <textarea
                  ...
                  onChange={(e) => setCompetitorFeatures(e.target.value)}
                  className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl p-3 text-xs text-slate-200 placeholder-slate-500 focus:outline-none min-h-[100px] leading-relaxed"
                  placeholder={t('competitorFeaturesPlaceholder')}
                />
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="relative">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onFocus={() => ourProduct.length > 0 && setShowProductDropdown(true)}
                      className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl pl-8 pr-3 py-2.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none font-bold"
                      placeholder={t('ourProductPlaceholder')}
                    />
                  ...
                  {showProductDropdown && filteredProducts.length > 0 && (
                    <div className="absolute z-[99] mt-1.5 w-full bg-slate-900 border border-slate-800 shadow-xl rounded-xl overflow-hidden divide-y divide-slate-850">
                      {filteredProducts.map(p => (
                        <div
                          key={p.id}
                          onClick={() => selectProduct(p)}
                          className="px-3 py-2.5 text-xs text-slate-350 hover:bg-slate-850 hover:text-white cursor-pointer transition-colors flex items-center justify-between"
                        >
                          <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2.5 text-xs text-slate-205 placeholder-slate-500 focus:outline-none"
                    placeholder="e.g. Exercise Cycles"
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    type="button"
                    onClick={() => handleDictation('ourFeatures', ourFeatures, setOurFeatures)}
                    className={`px-2 py-1 rounded-lg flex items-center gap-1 text-[10px] font-bold border transition-colors ${
                      recordingField === 'ourFeatures'
                        ? 'bg-red-500/20 border-red-500/30 text-red-400 animate-pulse'
                        : 'bg-slate-950 border-slate-850 text-slate-400 hover:text-indigo-400 hover:border-indigo-500/20'
                    }`}
                  >
                ...
                <textarea
                  value={ourFeatures}
                  onChange={(e) => setOurFeatures(e.target.value)}
                  className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl p-3 text-xs text-slate-200 placeholder-slate-500 focus:outline-none min-h-[100px] leading-relaxed"
                  placeholder={t('ourProductFeaturesPlaceholder')}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    type="button"
                    onClick={() => handleDictation('customerRequirements', customerRequirements, setCustomerRequirements)}
                    className={`p-1 rounded-lg transition-colors ${
                      recordingField === 'customerRequirements' ? 'bg-red-500/20 text-red-500 animate-pulse' : 'text-slate-500 hover:text-indigo-400'
                    }`}
                    title="Speak requirements"
                  >
                    <Mic className="w-3.5 h-3.5" />
                  </button>
                </div>
                <input
                  ...
                  className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2.5 text-xs text-slate-200 placeholder-slate-550 focus:outline-none"
                  placeholder={t('customerRequirementsPlaceholder')}
                />
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl pl-7 pr-3 py-2.5 text-xs text-slate-200 placeholder-slate-550 focus:outline-none"
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      type="button"
                      onClick={() => handleDictation('intendedUsage', intendedUsage, setIntendedUsage)}
                      className={`p-1 rounded-lg transition-colors ${
                        recordingField === 'intendedUsage' ? 'bg-red-500/20 text-red-500 animate-pulse' : 'text-slate-500 hover:text-indigo-400'
                      }`}
                      title="Speak intended usage"
                    >
                      <Mic className="w-3.5 h-3.5" />
                    </button>
                  </div>
                  <input
                    ...
                    className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2.5 text-xs text-slate-200 placeholder-slate-550 focus:outline-none"
                    placeholder={t('intendedUsagePlaceholder')}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    type="button"
                    onClick={() => handleDictation('additionalNotes', additionalNotes, setAdditionalNotes)}
                    className={`p-1 rounded-lg transition-colors ${
                      recordingField === 'additionalNotes' ? 'bg-red-500/20 text-red-500 animate-pulse' : 'text-slate-500 hover:text-indigo-400'
                    }`}
                    title="Speak notes"
                  >
                    <Mic className="w-3.5 h-3.5" />
                ...
                <input
                  ...
                  className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2.5 text-xs text-slate-200 placeholder-slate-550 focus:outline-none"
                  placeholder={t('auxiliaryContextPlaceholder')}
                />
              </div>
            </div>
          ...
          <div className="pt-6 border-t border-slate-850/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2.5 text-xs text-slate-300 focus:outline-none cursor-pointer"
              >
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="submit"
                className="bg-indigo-650 hover:bg-indigo-755 text-white font-extrabold px-8 py-4 rounded-xl shadow-lg shadow-indigo-600/15 hover:shadow-indigo-600/25 transition-all text-sm cursor-pointer"
              >
                {t('generateBtn')}
              </button>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl animate-fade-in">
            ...
            <form onSubmit={handleSaveAsTemplate} className="space-y-4">
              <input
                type="text"
                required
                value={newTemplateSaveName}
                onChange={(e) => setNewTemplateSaveName(e.target.value)}
                placeholder="e.g. iPhone 15 vs Galaxy S24 comparison"
                className="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-3 text-xs text-slate-205 placeholder-slate-500 focus:outline-none"
                autoFocus
              />
              ...
              <div className="flex gap-3 justify-end pt-2 text-xs">
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-750 text-white font-bold rounded-xl transition-colors disabled:opacity-50 cursor-pointer"
                >
