  ...
  return (
    <div className="space-y-6">
        ...
        <form onSubmit={handleSearchSubmit} className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
          <div className="flex-1 relative">
            <input
              ...
              className="block w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl pl-9 pr-3 py-2.5 text-xs text-slate-205 placeholder-slate-500 focus:outline-none"
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs text-slate-300 focus:outline-none cursor-pointer"
              >
            ...
            <button
              type="submit"
              className="px-4 py-2 bg-slate-850 hover:bg-slate-700 text-slate-200 text-xs font-semibold rounded-xl transition-colors cursor-pointer"
            >
              {t('applyFilterBtn')}
            </button>
            {(search || ratingFilter) && (
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        {loading ? (
          <div className="p-8 space-y-4">
            {[1, 2, 3, 4].map(n => (
              <div key={n} className="h-14 bg-slate-950/40 rounded-xl animate-pulse" />
            ))}
          </div>
                  ...
                  <tr key={row.id} className="hover:bg-slate-850/30 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      {row.rating ? (
                        ...
                        <span className="text-slate-650">{t('unrated')}</span>
                      )}
                    </td>
                    <td className="px-5 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          ...
                          className="p-1.5 text-indigo-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors cursor-pointer"
                          title="View brief output"
                        >
                          <Eye className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => setConfirmDeleteId(row.id)}
                          className="p-1.5 text-red-500 hover:text-white hover:bg-red-500/20 rounded-lg transition-colors cursor-pointer"
      ...
      {!loading && totalPages > 1 && (
        <div className="flex items-center justify-between no-print px-1 py-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="p-1.5 bg-slate-900 border border-slate-800 hover:bg-slate-850 rounded-lg text-slate-300 disabled:opacity-40 transition-colors cursor-pointer"
            >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-2xl animate-scale-up">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex-1 py-2 border border-slate-800 hover:bg-slate-850 rounded-xl text-xs font-semibold text-slate-350 transition-colors cursor-pointer"
              >
                {t('cancelBtn')}
              </button>
              <button
                onClick={() => confirmDeleteId !== null && handleDelete(confirmDeleteId)}
                className="flex-1 py-2 bg-red-650 hover:bg-red-750 text-white text-xs font-bold rounded-xl transition-all cursor-pointer"
              >
                {t('deleteBtn')}
              </button>
            </div>
